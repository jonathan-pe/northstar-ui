...
- `src/index.css`
  - app entry: imports Tailwind + `src/styles/theme.css`
- `src/styles/theme.css`
  - shadcn/Tailwind theme tokens, `@theme inline`, and base styles; **canonical `:root` theme custom properties**
- `src/styles/overrides.css`
  - project-owned CSS loaded after `theme.css`; **override or add theme vars here** (merged into generated TS)
...
3. Keep CSS layering explicit:
  - base/upstream: `src/styles/theme.css`
  - custom overrides: `src/styles/overrides.css`
  - published to consumers: `dist/theme.css`, `dist/styles.css` (via `scripts/build-styles.ts`)
...
Primitives (stable wrappers in `src/primitives/wrappers/*` plus direction):

- Alert Dialog, Avatar, Badge, Button, Card, Checkbox, Dialog, Dropdown Menu, Form, Input, Input OTP, Label, Progress, Select, Separator, Sheet, Sidebar, Skeleton, Switch, Table, Tabs, Textarea, Toast (Sonner), Toggle, Tooltip
- Direction: `DirectionProvider`, `useDirection` (implemented in `src/components/ui/direction.tsx`, exported from `northstar-ui/primitives`)

Composites:

- `LoginCard`, `AppSidebar` (`src/composites/*`)
...
- **Canonical catalog (TypeScript)**: `src/stories/catalog/` — primitive/composite entries, tokens block, and `catalogStoryDocs()` for Storybook doc blocks.
- **Generated JSON**: `docs/agent/northstar-catalog.json` via `pnpm generate:agent-catalog` (also invoked by `pnpm build:lib`) for agents, MCP, and published `docs/` artifacts.
- **Drift check**: `pnpm verify:agent-catalog` ensures the JSON matches TS and that wrappers/composites stay aligned (CI runs this).
- **Cursor skill**: `.cursor/skills/northstar-ui/SKILL.md` teaches assistants the import contracts and architecture rules.
- **MCP server (stdio)**: `pnpm mcp:northstar` serves catalog tools + `northstar://catalog` resource; details in `docs/agent/README.md`.
...
- `src/stories/catalog/*` when adding/renaming primitives, composites, or materially changing import examples or composition guidance; then run `pnpm generate:agent-catalog` and `pnpm verify:agent-catalog`